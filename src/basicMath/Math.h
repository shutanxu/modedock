...
const static double vdata[12][3] =  {
		{-X, 0.0, Z}, {X, 0.0, Z}, {-X, 0.0, -Z}, {X, 0.0, -Z},
		{0.0, Z, X}, {0.0, Z, -X}, {0.0, -Z, X}, {0.0, -Z, -X},
		{Z, X, 0.0}, {-Z, X, 0.0}, {Z, -X, 0.0}, {-Z, -X, 0.0}
};

const static unsigned short tindices[20][3] = {
...
const static unsigned short icosahedronEdge[30][2] = {
		{0, 1}, {0, 4}, {0, 6}, {0, 9}, {0, 11}, {1, 4}, {1, 6}, {1, 8},
		{1, 10}, {2, 3}, {2, 5}, {2, 7}, {2, 9}, {2, 11}, {3, 5}, {3, 7},
		{3, 8}, {3, 10}, {4, 5}, {4, 8}, {4, 9}, {5, 8}, {5, 9}, {6, 7},
...
//	v2Len = sqrt(v2Len);
	if ( v1Len < Tiny || v2Len < Tiny){
		std::cout<<"divide by ZERO in interAngle \n";
		exit(1);
	}
...
inline double* polarCoord(double *vec, int dim) {
	...
	if ( dim != 3 ){
		std::cout<<"This polar function only for 3D \n";
		exit(1);
	}
...
inline void Diff( const double *const x, const double *const y,
		double *z, const size_t dim = 3){
	for ( unsigned int i = 0; i < dim; i++ )
		z[i]= x[i] - y[i];
}

inline double Dot(const double *const x, const double *const y) {
	return x[0] * y[0] + x[1] * y[1] + x[2] * y[2];
}

inline void Cross(const double *const x,  const double *const y, double *z){
	z[0] = x[1] * y[2] - y[1] * x[2];
	z[1] = x[2] * y[0] - y[2] * x[0];
	z[2] = x[0] * y[1] - y[0] * x[1];
}
