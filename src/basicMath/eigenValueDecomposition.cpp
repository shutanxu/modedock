...
/**
 * The variables that need to be initialized are
 * $d, V, e$
 */
void eigenValueDecomposition::tred2 () {
 ...
 /*
  * The variables that need to be initialized are $d, V, e$ that have been
  * computed in _tred2_
  * */
 void eigenValueDecomposition::tql2 () {
   ...
   //  Fortran subroutine in EISPACK.
      ...
      double f = 0.0;
      double tst1 = 0.0;
      double epsL = pow(2.0,-52.0);
      for (int l = 0; l < n; l++) {

         // Find small subdiagonal element

         tst1 = dMax(tst1, fabs(d[l]) + fabs(e[l]));
         int m = l;
         while (m < n) {
            if ( fabs(e[m]) <= epsL*tst1) {
               break;
            }
            m++;
         }
         ...
         // No convergence yet
            ...
            while (m >= l) {
               ...
               if (fabs(H[m][m-1]) * (fabs(q) + fabs(r)) <
                  epsL * (fabs(p) * (fabs(H[m-1][m-1]) + fabs(z) +
                  fabs(H[m+1][m+1])))) {
                     break;
               }
               m--;
 ...
 * ------------------------ */
  ...
  eigenValueDecomposition::eigenValueDecomposition (matrix *Arg, bool issymmetric) {
 ...
 matrix* eigenValueDecomposition::getD() const{
	 matrix *X = new matrix(n, n);
	 double **D = X->getArrayCopy();
	 for (int i = 0; i < n; i++) {
		 for (int j = 0; j < n; j++)
			 D[i][j] = 0.0;
		 D[i][i] = d[i];
